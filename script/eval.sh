...
SAMPLE_FLAGS="--batch_size ${BATCH_SIZE} --batch_size_eval ${BATCH_SIZE} --num_samples 10000 --mode_evaluation ${EVALUATION} --timestep_respacing 250" # --timestep_respacing 250
...
python eval.py $MODEL_FLAGS $SAMPLE_FLAGS $ENCODER_FLAGS
...
case $RUN in
    ...
    vfsddpm_omniglot_vit_lag_meanpatch_sigma_outdistro)
      run 28 1 vfsddpm omniglot_back_eval vit lag mean_patch deterministic out-distro 7 5 64
      ;;
    vfsddpm_omniglot_vit_lag_meanpatch_sigma_outdistro_10)
      run 28 1 vfsddpm omniglot_back_eval vit lag mean_patch deterministic out-distro 7 10 64
      ;;
